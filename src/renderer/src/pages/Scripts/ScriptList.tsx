      ...
      <ResizablePanelGroup direction="horizontal" className="min-h-[calc(100vh-2rem)] rounded-lg">
        <ResizablePanel defaultSize={60} minSize={40} maxSize={60} className="min-h-full">
          <div className="flex flex-col gap-2 h-full p-2">
            <ScriptHeader onNewScript={handleNewScript} />
                ...
                <TableBody>
                  {creatingNewScript && (
                    <NewScriptRow
                      ...
                      onKeyDown={(e) => e.key === 'Enter' && handleNewScriptSave(true)}
                      showError={showErrorTooltip}
                    />
                  )}
                  {reversedScripts.map((script) => (
                    <ScriptRow
                      key={script.id}
                      script={script}
                      isEditing={editableScriptId === script.id}
                      editName={newScriptName}
                      onEdit={() => handleEditClick(script.id, script.name)}
                      onDelete={() => deleteScript(script.id)}
                      onNameChange={handleNameChange}
                      onNameSave={() => handleNameSave(script.id)}
                    />
                  ))}
                </TableBody>
              </Table>
